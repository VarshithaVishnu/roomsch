...
@app.post("/add_room", response_class=RedirectResponse)
    ...
    if id_token:
        try:
            ...
            if not any(existing_room):
                ...
                firestore_db.collection('rooms').add(ev_data)
            else:
...
@app.get("/list_room", response_class=HTMLResponse)
async def list_rooms(request: Request):
    ...
    return templates.TemplateResponse("room_list.html", {"request": request, "rooms": rooms_data})

@app.get("/add_booking", response_class=HTMLResponse)
async def add_booking(request: Request, error: str = None):
    rooms_query = firestore_db.collection('rooms').stream()
    rooms = [{'id': room.id, 'name': room.to_dict().get('name', '')} for room in rooms_query]
    return templates.TemplateResponse('add_booking.html', {'request': request, 'rooms': rooms, 'error': error})

@app.post("/add_booking", response_class=RedirectResponse)
async def add_booking(request: Request, customer_name: str = Form(...), room: str = Form(...), start_date: str = Form(...), start_time: str = Form(...), end_date: str = Form(...), end_time: str = Form(...)):
    ...
    for booking in bookings_query:
        ...
        if not (end_datetime <= existing_start or start_datetime >= existing_end):
            return RedirectResponse(url=f"/add_booking?error=Booking+already+exists+for+given+time+period,+try+another", status_code=status.HTTP_303_SEE_OTHER)
    id_token = request.cookies.get("token")
    user_token = None
    if id_token:
        try:
...
@app.get("/list_booking", response_class=HTMLResponse)
async def list_bookings(request: Request):
    ...
    return templates.TemplateResponse("booking_list.html", {"request": request, "bookings": bookings_data})
...
async def delete_booking(booking_id: str):
    ...
    booking_ref.delete()
    return RedirectResponse("/list_booking", status_code=status.HTTP_303_SEE_OTHER)
...
async def edit_booking(request: Request, booking_id: str):
    ...
    if booking_doc.exists:
        ...
        return templates.TemplateResponse("edit_booking.html", {"request": request, "booking": booking, 'rooms': rooms,})
    else:
        return RedirectResponse(url="/", status_code=status.HTTP_302_FOUND)
    
@app.post("/update_booking", response_class=RedirectResponse)
async def update_booking(request: Request, booking_id: str = Form(...), customer_name: str = Form(...), room: str = Form(...), start_date: str = Form(...), start_time: str = Form(...), end_date: str = Form(...), end_time: str = Form(...)):
    ...
    return RedirectResponse(url="/list_booking", status_code=status.HTTP_303_SEE_OTHER)
...
async def delete_room(room_id: str, request: Request):
    ...
    if not date_matched and user_id:
        ...
        if room_data and room_data.get('user_id') == user_id:
            ...
            return {"message": "Unauthorized to delete the room"}
    
    return {"message": "No action taken"}
    
@app.get("/filter_booking", response_class=HTMLResponse)
...
@app.get("/filter_by_room/{room_name}", response_class=HTMLResponse)
async def edit_booking(request: Request, room_name: str):
    ...
    return templates.TemplateResponse("filter_by_room.html", {"request": request, "bookings": bookings})